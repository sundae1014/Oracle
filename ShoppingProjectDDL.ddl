...
CREATE TABLE ordersitem (
    ...
    itemcount    NUMBER DEFAULT 1 NOT NULL
);
...
CREATE TABLE product (
    ...
    cateno       NUMBER(2) NOT NULL,
    sellerno     NUMBER(5),
    prodname     VARCHAR2(200 BYTE) NOT NULL,
...
order by o.ordertotalprice desc, name asc;
...
from product
group by prodno, prodname, prodprice, prodDiscount;
...
select 
    ...
    s.tel
from seller s
join product p on p.sellerNo = s.sellerNo
where p.sellerNo is null;

--문제 9
select
    orderno,
    sum(itemprice * itemcount * (1 - itemdiscount / 100)) as 최종총합
from ordersitem
group by orderno
having sum(itemprice * itemcount * (1 - itemdiscount / 100)) >= 100000
order by 최종총합 desc;
